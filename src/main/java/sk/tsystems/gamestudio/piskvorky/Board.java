...
//        }
    ...
    private boolean condition(int x, int y, int nbOfNeighbours, String dir, int inRow, Symbol.State state) {
        return onBoard(x, y) && board[x][y].getState() == state && inRow < nbOfNeighbours && !isCrossedInDirection(x, y, dir);
    }

    private boolean isCrossedInDirection(int x, int y, String dir) {
